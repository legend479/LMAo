    ...
    async def _create_enhanced_plan(
        ...
        for i, (tool_name, tool_params) in enumerate(intent_match.tool_sequence):
            ...
            if i > 0:
                ...
                elif tool_params.get("attach_previous"):
                    # When email is meant to attach the output of the previous task
                    # (typically document_generation), inject an attachments placeholder
                    # that the orchestrator will resolve to the previous task's file path
                    prev_task_id = f"task_{i}"
                    attachments_placeholder = (
                        f"{{{{ {prev_task_id}.result.file_path }}}}"
                    )
                    # Ensure attachments is a list in parameters
                    existing_attachments = parameters.get("attachments") or []
                    if not isinstance(existing_attachments, list):
                        existing_attachments = [existing_attachments]
                    existing_attachments.append(attachments_placeholder)
                    parameters["attachments"] = existing_attachments
                    # Also set a simple body placeholder if no explicit body provided
                    if "body" not in parameters:
                        parameters["body"] = (
                            f"Please find the attached document generated by the agent: {{{{ {prev_task_id}.result.filename }}}}."
                        )
                    logger.debug(
                        "Set attachment placeholder for email task",
                        task_id=task_id,
                        prev_task_id=prev_task_id,
